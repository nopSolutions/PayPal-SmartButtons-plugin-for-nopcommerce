...
namespace Nop.Plugin.Payments.PayPalSmartPaymentButtons.Services
{
    ...
    public class ServiceManager
    {
            ...
            PayPalSmartPaymentButtonsSettings settings) where TResource : class
        {
            ...
            var (response, errorMessage) = HandleRequest<PayPal.Core.PayPalHttpClient, WebhookVerifySignatureRequest<TResource>, VerifyWebhookSignatureResponse>(client, verifyRequest, settings);
        ...
        public (Order order, string errorMessage) CreateOrder(Guid orderGuid)
        {
            ...
            orderDetails.Payer = new Payer
            {
                ...
                AddressPortable = new AddressPortable
                {
                    ...
                    AdminArea1 = CommonHelper.EnsureMaximumLength(billingAddress.StateProvince?.Abbreviation, 300),
            ...
            {
                var itemPrice = Math.Round(_taxService.GetProductPrice(item.Product,
                    _priceCalculationService.GetUnitPrice(item), false, _workContext.CurrentCustomer, out _), 2);
                itemTotal += itemPrice * item.Quantity;
                return new Item
                {
                    Name = CommonHelper.EnsureMaximumLength(item.Product?.Name, 127),
                    Description = CommonHelper.EnsureMaximumLength(item.Product?.ShortDescription, 127),
                    Sku = CommonHelper.EnsureMaximumLength(item.Product?.Sku, 127),
                    Quantity = item.Quantity.ToString(),
                    Category = ((item.Product?.IsDownload ?? false) ? ItemCategoryType.Digital_goods : ItemCategoryType.Physical_goods)
                        .ToString().ToUpper(),
            ...
            var attributesValues = _checkoutAttributeParser.ParseCheckoutAttributeValues(_genericAttributeService
            ...
            {
                if (value.CheckoutAttribute == null)
                    continue;

                var attributePrice = Math.Round(_taxService.GetCheckoutAttributePrice(value, false, _workContext.CurrentCustomer), 2);
                itemTotal += attributePrice;
                purchaseUnit.Items.Add(new Item
                {
                    Name = CommonHelper.EnsureMaximumLength(value.CheckoutAttribute.Name, 127),
                    Description = CommonHelper.EnsureMaximumLength($"{value.CheckoutAttribute.Name} - {value.Name}", 127),
                    Quantity = 1.ToString(),
                    UnitAmount = new PayPalCheckoutSdk.Orders.Money { CurrencyCode = currency, Value = attributePrice.ToString("F") }
                });
            }
            ...
            return HandleRequest<PayPalHttpClient, OrdersCreateRequest, Order>(_client, new OrdersCreateRequest { Body = orderDetails });
        }
        ...
        public (Webhook webhook, string errorMessage) CreateWebHook(PayPalSmartPaymentButtonsSettings settings, string url)
        {
            ...
            return HandleRequest<PayPal.Core.PayPalHttpClient, WebhookCreateRequest, Webhook>(client, createRequest, settings);
        }
